...
module.exports.createPost = async (req,res)=>{
    ...
    if (existingProduct.length > 0) {
        ...
        req.body.watched = 0
        try {
            await db.execute(`insert into product (name,author,description,categories,isbn,publisher,publish_year,language,price,stock,image,watched,page,status) 
                                values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
                                [req.body.name,req.body.author,req.body.description,req.body.categories,req.body.isbn,req.body.publisher,req.body.publish_year,req.body.language,req.body.price,req.body.stock,req.body.image,req.body.watched,req.body.page,req.body.status]);
...
module.exports.editPost = async (req,res)=>{
    ...
    await db.execute(`update product set name = ?,author = ?,description = ?,categories = ?,publisher = ?,publish_year = ?,language = ?,price = ?,stock = ?,page = ?,status = ? where isbn = ?`,
                    [req.body.name,req.body.author,req.body.description,req.body.categories,req.body.publisher,req.body.publish_year,req.body.language,req.body.price,req.body.stock,req.body.page,req.body.status,isbn])
